...
def compare_string(string1,string2,nomatch=0,i=0,d=0):
...
def compare_files(file,file2,nomatch=0,i=0,d=0):
    count=0
            ...
            for string1 in string1_file:
                with open(file2, 'r') as string2_file:
                    ...
                    for string2 in string2_file:
                        toggle=compare_string(string1.strip(),string2.strip(),nomatch,i,d)
                        if toggle==1:
                            match=1
                            count+=1
...
def parse_args():
    ...
    parser.add_argument("-m", "--matchonly", help="Print only a list of matching strings", action="store_true")
    parser.add_argument("-d", "--dontmatch", help="Print only a list of strings that don't match", action="store_true")
...
def main():
    args=parse_args()
    compare_files(args.source,args.dest,args.nomatch,args.matchonly,args.dontmatch)
